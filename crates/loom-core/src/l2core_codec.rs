...
pub enum L2CoreCodecError {
    ...
    InvalidUtf8,
    /// An Arrow [`DataType`] variant is not supported by this codec slice.
    UnsupportedDataType,
...
impl std::fmt::Display for L2CoreCodecError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            L2CoreCodecError::UnsupportedDataType => write!(f, "unsupported DataType"),
...
// DataType — narrow L2Core-supported subset
...
fn write_data_type(buf: &mut Vec<u8>, dt: &DataType) {
    let disc = match dt {
        ...
        DataType::Float64 => 4,
        DataType::Utf8 => 5,
        _ => panic!("L2CoreCodec: unsupported DataType {dt:?} — only Boolean/Int32/Int64/Float32/Float64/Utf8 are supported"),
    };
    write_u8(buf, disc);
}

fn read_data_type(cur: &mut Cursor<&[u8]>) -> Result<DataType, L2CoreCodecError> {
    match read_u8(cur)? {
...
mod tests {
    ...
    fn mk_expr(depth: usize, arrow_type: &DataType) -> ScalarExpr {
        if depth == 0 {
            return match arrow_type {
                DataType::Boolean => ScalarExpr::Const(ScalarValue::Bool(true)),
                DataType::Int32 => ScalarExpr::Const(ScalarValue::Int32(42)),
                DataType::Int64 => ScalarExpr::Const(ScalarValue::Int64(42)),
                DataType::Float32 => ScalarExpr::Const(ScalarValue::Float32Bits(3.14f32.to_bits())),
                DataType::Float64 => ScalarExpr::Const(ScalarValue::Float64Bits(3.14f64.to_bits())),
                DataType::Utf8 => ScalarExpr::Const(ScalarValue::Bytes(vec![0xAB, 0xCD])),
