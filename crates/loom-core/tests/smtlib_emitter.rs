...
fn script_uses_stable_symbols_and_named_bad_states() {
    ...
    assert!(script
        .text
        .contains(":named loom_bad___body_0__read_add_no_overflow_0"));
    assert!(script
        .text
        .contains(":named loom_bad___body_0__read_in_range_1"));
    assert!(script
        .text
        .contains(":named loom_bad___body_1__cursor_decreases_2"));
}
...
fn offset_width_bounds_include_unsigned_overflow_bad_state() {
    ...
    assert!(script
        .text
        .contains("(bvadd loom_input_offset loom_read_width)"));
    assert!(script
        .text
