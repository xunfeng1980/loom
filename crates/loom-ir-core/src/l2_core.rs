...
//!
//! This is the Arrow-independent copy used by `loom-ir-core`. The `L2DataType`
//! enum replaces `arrow_schema::DataType` so that the IR layer has zero Arrow
//! dependencies.

pub mod constraints;

/// The narrow set of Arrow data types supported by the L2Core IR.
///
/// This is a local type defined in `loom-ir-core` so that the IR layer
