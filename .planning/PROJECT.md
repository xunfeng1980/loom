...
- ✓ Lean modeled operational semantics and soundness theorem: `formal/lean/LoomCore.lean` now contains a proof-friendly modeled executor, typed modeled events, fail-closed terminal semantics, modeled safety predicates that read `execProgram p` state evidence (`readsInBounds`, `eventsTyped`, `rowsWithinMax`, terminal status), and a no-`sorry` semantic `accepted_program_safe : Verified p -> ModeledExecutionSafe p` theorem that consumes both modeled execution evidence and the `Verified p` premises; `scripts/full-verifier-test.sh` checks the theorem marker, state-evidence bridge markers, modeled-only scope note, no `_state`/discarded-premise regression, and no-sorry policy — Phase 38
