...
key-files:
  ...
  modified:
    ...
    - Cargo.lock
key-decisions:
  - "Keep Phase 28 Iceberg vocabulary adapter-local and out of core/FFI/source-ingress/public surfaces."
  - "Pin only serde_json = =1.0.150 for local JSON parsing; do not add the official iceberg SDK by default."
  - "Create the focused Iceberg binding gate but leave it unwired from mvp0-verify until later Phase 28 closeout."
...
# Phase 28 Plan 01: Iceberg Binding Scaffold Summary
...
- `scripts/iceberg-binding-test.sh` - Added the focused Phase 28 dependency/scope guard.

## Decisions Made

- Followed the Phase 28 default: no official `iceberg` crate in the default graph.
...
- Left `scripts/mvp0-verify.sh` unchanged because Plan 28-01 explicitly creates but does not wire the focused gate.
...
- `.planning/STATE.md` already contained a Phase 28 execution-start edit before this plan completed; the update preserves that direction and advances only the current plan/session fields.
...
Plan 28-02 can add local Iceberg metadata and sidecar fixture parsing on top of the established crate, report types, and dependency guards. The focused gate is intentionally available but not yet part of the main release verifier.
