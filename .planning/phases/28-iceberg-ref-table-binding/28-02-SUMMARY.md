...
key-decisions:
  - "Keep Plan 28-02 parser output descriptive only; verifier/hash/source/oracle acceptance remains deferred to Plan 28-03."
...
# Phase 28 Plan 02: Iceberg Local Parser Summary
...
- Kept all Plan 28-02 outputs descriptive: no artifact bytes are emitted, no hashes are recomputed, `verify_artifact` is not called, and sidecar verifier/oracle claims do not construct accepted bindings.
...
- **Found during:** Task 2
- **Issue:** The initial accepted metadata fixture used `warehouse` in its local path, but Plan 28-02 treats `warehouse` as a catalog/control marker.
...
Plan 28-03 can validate referenced artifact bytes, recompute hashes, run verifier acceptance, and compare source/oracle evidence against the descriptive facts created here.
