...
key-decisions:
  ...
  - "The focused Phase 28 gate runs accepted handoff and artifact verifier tests but remains unwired from mvp0-verify until Plan 28-05."
...
# Phase 28 Plan 03: Accepted Binding Validation Summary
...
- Preserved Plan 28-03 scope by leaving `scripts/mvp0-verify.sh` unwired and adding no Iceberg SDK, catalog, object-store, SQL, CLI, C ABI, DuckDB, or StarRocks route surface.
...
Plan 28-04 can build on a fail-closed accepted binding function and focused gate evidence. Accepted bindings now require concrete local artifact bytes, live verifier acceptance, recomputed SHA-256, and matching sidecar-referenced source/oracle evidence before bytes are returned.
