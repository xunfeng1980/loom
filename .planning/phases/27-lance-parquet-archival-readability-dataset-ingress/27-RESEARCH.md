...
1. **Which legacy source versions should be represented?**  
   ...
   Recommended default: Use one tiny checked-in fixture per family generated by the oldest version that can be produced without remote services or brittle build steps during implementation; record exact generator versions in the final report. [ASSUMED]
...
2. **Should Lance adapter APIs be async-only or expose a test-only sync wrapper?**  
   ...
   Recommended default: Keep Lance adapter functions async and use `#[tokio::test]` in tests; do not create a public blocking wrapper. [CITED: https://docs.rs/tokio/latest/tokio/; ASSUMED]
