...
| VERIFIER-07 | Local arithmetic, range, overflow, loop-variant, and resource-bound obligations are represented as SMT-ready constraints. | SMT/Rust | `13-VERIFIER-SPEC.md` static semantics and resource model; `crates/loom-core/src/l2_core/constraints.rs` | `cargo test -p loom-core --test l2_core_model` checks `AddNoOverflow`, `InRange`, and `Decreases` diagnostic text | `13-02` model gate, then future full-verifier gate | Constraint IR evidence added in 13-02 | Solver integration remains intentionally deferred. |
...
| VERIFIER-10 | Phase 13 emits verifier facts/proof obligations that Phase 14 can use as native-lowering preconditions. | Gate/Rust | `13-VERIFIER-SPEC.md` sections `VerifiedArtifactFacts` and `Lowering Preconditions`; `VerifiedArtifactFacts` in `crates/loom-core/src/l2_core.rs` | `cargo test -p loom-core --test l2_core_model` checks row count, input range, output schema, loop bound, constraint IDs, and proof IDs | `13-01` facts definition, `13-02` fact model gate, then `13-05` final closeout | Fact model evidence added in 13-02 | Final Phase 14 handoff report remains planned. |
