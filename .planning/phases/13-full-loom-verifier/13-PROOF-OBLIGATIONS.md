...
| VERIFIER-09 | A Lean or Rocq proof scaffold defines core semantics and states or proves an accepted-program safety theorem. | Lean/Rocq | `formal/lean/LoomCore.lean` defines `builder_events_well_formed` and `accepted_program_safe` | `bash scripts/full-verifier-test.sh` checks Lean theorem names and runs Lean when installed | `scripts/full-verifier-test.sh` | Complete | Scaffold is intentionally narrow; complete final Loom soundness remains future work. |
