...
| VERIFIER-03 | `L2Core` syntax, static semantics, dynamic semantics, and allowed loop forms are defined. | Spec | `13-VERIFIER-SPEC.md` sections `L2Core Syntax`, `Static Semantics`, and `Dynamic Semantics` | `rg -n "ForRange\|CursorLoop\|Static Semantics" 13-VERIFIER-SPEC.md` | `13-01` docs gate, then `13-02/13-04` model gates | Planned in 13-01 | Rust model and Lean/Rocq model still need to mirror the spec. |
| VERIFIER-04 | The capability/resource model covers input ranges, scratch bounds, output builders, no ambient authority, and fail-closed errors. | Spec | `13-VERIFIER-SPEC.md` sections `Capability Model` and `Resource Model` | `rg -n "no ambient authority\|ResourceBudget\|FailClosed" 13-VERIFIER-SPEC.md` | `13-01` docs gate, then `13-03` verifier tests | Planned in 13-01 | Abstract interpretation and diagnostics are planned in `13-03`. |
| VERIFIER-05 | Arrow builder event semantics are specified so output well-formedness can be checked or proved by construction. | Spec | `13-VERIFIER-SPEC.md` section `Arrow Builder Event Semantics` | `rg -n "BuilderEvent\|AppendValue\|AppendNull" 13-VERIFIER-SPEC.md` | `13-01` docs gate, then `13-04` formal scaffold gate | Planned in 13-01 | Lean/Rocq builder-event theorem scaffold remains planned. |
| VERIFIER-06 | A Rust verifier prototype or architecture uses type/effect checking plus abstract interpretation for `L2Core`. | Rust | `13-VERIFIER-SPEC.md` type/effect judgment and `13-PATTERNS.md` verifier module pattern | Future `crates/loom-core` tests for accepted/rejected `L2Core` programs | Future `scripts/full-verifier-test.sh` | Planned in 13-02/13-03 | No Rust `L2Core` executable verifier exists yet. |
| VERIFIER-07 | Local arithmetic, range, overflow, loop-variant, and resource-bound obligations are represented as SMT-ready constraints. | SMT | `13-VERIFIER-SPEC.md` static semantics and resource model | Future constraint IR tests checking emitted obligation IDs | Future full-verifier gate | Planned in 13-02/13-03 | Constraint IR and optional SMT-LIB rendering are not implemented yet. |
| VERIFIER-08 | Verifier diagnostics and proof-obligation traces are stable enough for reviewer-facing rejection reports. | Rust | Existing Phase 9/12 diagnostic pattern in `13-PATTERNS.md`; `13-VERIFIER-SPEC.md` fail-closed contract | Future rejection tests that assert stable diagnostic codes and obligation traces | Future full-verifier gate and final report | Planned in 13-02/13-03 | Diagnostic codes and trace format need implementation. |
...
| VERIFIER-10 | Phase 13 emits verifier facts/proof obligations that Phase 14 can use as native-lowering preconditions. | Gate | `13-VERIFIER-SPEC.md` sections `VerifiedArtifactFacts` and `Lowering Preconditions` | Future Rust facts tests and final doc checks | `13-01` facts definition, then `13-05` final closeout | Planned in 13-01/13-05 | Fact schema is specified now; executable fact emission and final Phase 14 handoff are later plans. |
