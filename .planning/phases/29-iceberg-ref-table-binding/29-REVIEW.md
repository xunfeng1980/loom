...
Reviewed the Phase 29 Iceberg binding crate, fixtures, dependency guards, release gate wiring, and DuckDB runtime FFI test isolation change. The workspace manifest and lockfile did not show an official `iceberg` crate dependency or Arrow 57/58 drift. The DuckDB runtime FFI cache isolation change is locally scoped and does not appear to mask the cache assertions in this test binary.
