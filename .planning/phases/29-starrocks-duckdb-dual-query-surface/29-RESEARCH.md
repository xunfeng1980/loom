# Phase 29: StarRocks + DuckDB Dual Query Surface - Research
...
|------------|--------------|----------------|-----------|
| Phase 28 binding acceptance | Adapter crate / Rust library | `loom-core` verifier | `loom-iceberg-binding` already owns local table/ref binding and calls `verify_artifact`; Phase 29 consumes accepted reports only. [VERIFIED: grep] |
...
| StarRocks-compatible evidence | Phase 29 adapter/report boundary | Optional external runtime smoke | StarRocks live catalog/load paths require cluster/network/credentials; default proof should be local descriptor validation. [CITED: https://docs.starrocks.io/docs/data_source/catalog/iceberg/iceberg_catalog/] |
| Query equivalence matrix | Phase 29 adapter crate | Scripts/report | The contract compares canonical rows/aggregates across surfaces without changing artifacts. [VERIFIED: grep] |
| Release gate ordering | Shell scripts | Cargo tests | Existing main gate runs Phase 28 before DuckDB smoke; Phase 29 should slot between them. [VERIFIED: grep] |
...
|----------------|---------|---------|--------------|
| `loom-iceberg-binding` | 0.1.0 | Accepted binding facts and verifier-backed artifact bytes | Already implements the Phase 28 trust root and mismatch fail-closed matrix. [VERIFIED: cargo metadata] |
...
| `loom-core` | 0.1.0 | Artifact verifier and decoded row checks | Phase 28 acceptance calls `verify_artifact` and decodes accepted values for evidence matching. [VERIFIED: cargo metadata] |
| `serde` / `serde_json` | `=1.0.228` / `=1.0.150` | Deterministic descriptor JSON parsing/emission | Already workspace-pinned and already used by the Phase 28 adapter. [VERIFIED: cargo metadata] |
...
| Docker | 29.2.0 installed locally | Optional runtime smoke orchestration | Available, but Phase 29 should not launch StarRocks by default because context forbids required live deployment. [VERIFIED: command] |
...
**Installation:**
```bash
# No new external package install is recommended for the default Phase 29 path.
```
...
```text
Phase 28 local metadata + sidecar + artifact path
        |
        v
...
├── src/lib.rs                         # exports bounded Phase 29 contract
...
.planning/phases/29-starrocks-duckdb-dual-query-surface/
└── 29-DUAL-QUERY-SURFACE-REPORT.md    # implementation closeout report
```

### Pattern 1: Accepted Binding as Sole Trust Root

**What:** Phase 29 should call `bind_iceberg_ref_from_paths` and proceed only on `IcebergBindingStatus::Accepted`. [VERIFIED: grep]
...
**Example:**
```rust
// Source: StarRocks SELECT docs + Phase 29 context [CITED: https://docs.starrocks.io/docs/sql-reference/sql-statements/table_bucket_part_index/SELECT/]
...
**Example:**
```bash
# Source: Phase 29 context + StarRocks Stream Load docs [CITED: https://docs.starrocks.io/docs/loading/StreamLoad/]
...
- **Generating query evidence from sidecar flags only:** Phase 28 proves sidecar flags are descriptive until independently checked. [VERIFIED: grep]
...
- **Creating a generic engine framework in Phase 29:** Context prefers adapter-local descriptors until the second surface shape is validated. [VERIFIED: grep]

## DuckDB / StarRocks Query Surface Comparison

| Dimension | DuckDB Required Surface | StarRocks Phase 29 Surface | Planning Implication |
|-----------|-------------------------|-----------------------------|----------------------|
...
| Iceberg route | Phase 28 binding only, not DuckDB Iceberg extension | StarRocks real external Iceberg catalog deferred | Real StarRocks Iceberg catalogs require metastore/storage access and credentials. [CITED: https://docs.starrocks.io/docs/data_source/catalog/iceberg/iceberg_catalog/] |
...
| Failure mode | DuckDB query mismatch fails focused gate | Descriptor mismatch or unsupported query fails closed | Both surfaces must preserve accepted Phase 28 identity. [VERIFIED: grep] |

## API Best Practices

- Keep Phase 29 public API local to `loom-dual-query-surface`; expose no C ABI and no CLI route. [VERIFIED: grep]
...
| Existing dirty worktree | Source files were modified before research started. [VERIFIED: git status] | Write only `29-RESEARCH.md`; do not modify source code. |

## Don't Hand-Roll

| Problem | Don't Build | Use Instead | Why |
|---------|-------------|-------------|-----|
| Artifact trust | A new query-surface verifier | `bind_iceberg_ref_from_paths` + `verify_artifact` evidence | Phase 28 already owns accepted bytes and mismatch failure. [VERIFIED: grep] |
...
| Engine framework | Generic query-engine abstraction in core | Adapter-local Phase 29 crate | Core/FFI/source-ingress must stay engine-neutral. [VERIFIED: grep] |

**Key insight:** Phase 29 is a trust-preserving equivalence proof, not a connector phase. The value is proving that one accepted artifact/binding identity can drive two query-surface descriptions without expanding runtime dependencies. [VERIFIED: grep]
...
**Why it happens:** StarRocks SELECT supports a broad SQL surface, but Phase 29 only proves a small matrix. [CITED: https://docs.starrocks.io/docs/sql-reference/sql-statements/table_bucket_part_index/SELECT/]
...
```rust
// Source: Phase 29 context + StarRocks SELECT docs
...
| Old / Larger Approach | Current Phase 29 Approach | When Changed / Source | Impact |
|-----------------------|---------------------------|-----------------------|--------|
| Real StarRocks Iceberg catalog proof | Offline descriptor over accepted Phase 28 binding | Context accepted 2026-06-09. [VERIFIED: grep] | Keeps release gate deterministic and credential-free. |
...
- Treating Phase 29 as completed evidence is outdated in current planning state; `.planning/STATE.md` records it as skipped/deferred, and this research artifact does not implement it. [VERIFIED: grep]
...
|---------|--------|---------------------|
| Forged sidecar or metadata acceptance | Spoofing/Tampering | Require accepted Phase 28 binding and recomputed hash/verifier/evidence checks. [VERIFIED: grep] |
...
- `.planning/phases/29-starrocks-duckdb-dual-query-surface/29-CONTEXT.md` - locked decisions, scope, tradeoffs, and deferred ideas. [VERIFIED: grep]
- `.planning/phases/29-starrocks-duckdb-dual-query-surface/29-PATTERNS.md` - closest existing code patterns and recommended file shape. [VERIFIED: grep]
- `.planning/phases/28-iceberg-ref-table-binding/28-ICEBERG-BINDING-REPORT.md` - Phase 28 accepted binding contract and handoff. [VERIFIED: grep]
...
- Standard stack: HIGH - derived from local cargo metadata, Phase 28 code, and official DuckDB/StarRocks docs. [VERIFIED: cargo metadata] [CITED: duckdb.org] [CITED: docs.starrocks.io]
- Architecture: HIGH - constrained by Phase 29 context and existing adapter/gate patterns. [VERIFIED: grep]
...
**Valid until:** 2026-06-16 for StarRocks runtime/version guidance; local adapter recommendations remain valid until Phase 29 context changes.
