...
|---|---|---|---|---|---|---|---|
| `OBL-12-01` | `loom-core` contains no unsafe code; C ABI unsafety is isolated in `loom-ffi`. | Rust core and FFI boundary | `crates/loom-core/src/lib.rs`, `crates/loom-ffi/src/lib.rs`, `crates/loom-ffi/src/ffi.rs` | Existing FFI tests; planned static check in `scripts/safety-proof-test.sh` | Planned `scripts/safety-proof-test.sh`; existing `scripts/check-core-invariants.sh` references `catch_unwind` | Planned | Add dedicated safety-proof static check. |
| `OBL-12-02` | `LMC1` container parsing rejects malformed headers, features, and section directories before wrapped payload decode. | Container bytes -> wrapped payload | `crates/loom-core/src/container_codec.rs`, `crates/loom-core/src/verifier.rs` | Existing `container_codec` tests; existing `scripts/container-negative-test.sh` | Existing `scripts/mvp0-verify.sh`; planned `scripts/safety-proof-test.sh` | Planned | Tie existing gate to this obligation and add proof-gate ID check. |
| `OBL-12-03` | Raw `LMP1`/`LMT1` payloads remain compatible but fail closed on parse errors. | Raw payload parse helpers | `crates/loom-core/src/layout_codec.rs`, `crates/loom-core/src/table_codec.rs`, `crates/loom-core/src/container_codec.rs` | Existing codec tests; planned malformed raw payload no-panic tests | Planned `scripts/safety-proof-test.sh` | Planned | Add focused public malformed raw tests. |
| `OBL-12-04` | Verifier diagnostics are typed, path-addressed, and exposed before successful decode output. | Verifier reports and CLI inspect | `crates/loom-core/src/verifier.rs`, `crates/loom-cli/src/main.rs` | Existing verifier unit tests; existing `scripts/verifier-negative-test.sh`; planned `safety_contract` tests | Existing `scripts/mvp0-verify.sh`; planned `scripts/safety-proof-test.sh` | Planned | Add obligation-linked test names and final proof references. |
| `OBL-12-05` | Decode helpers call verifier before Arrow output and return typed errors on verifier failure. | Decode helpers -> Arrow `ArrayData` | `crates/loom-core/src/l1_model.rs`, `crates/loom-core/src/table_codec.rs`, `crates/loom-ffi/src/ffi.rs` | Existing decode tests; planned decode-before-Arrow safety tests | Planned `scripts/safety-proof-test.sh` | Planned | Add tests that invalid verifier reports prevent `ArrayData` success. |
| `OBL-12-06` | Current parser/interpreter/kernel loops terminate because they are bounded by finite payload-derived counts or decoded array lengths. | Parser, verifier, interpreter, kernel loops | `container_codec.rs`, `layout_codec.rs`, `table_codec.rs`, `verifier.rs`, `l1_model.rs`, `l1_model/bitpack.rs`, `fsst_params.rs`, `alp_params.rs`, `l2_kernel_registry.rs` | Existing unit tests; planned no-panic malformed-count tests | Planned final proof and safety gate | Planned | Final `12-SAFETY-PROOF.md` must include loop-bound table. |
| `OBL-12-07` | L2 kernel params fail closed and kernel panics do not cross the public boundary. | `KernelEscape` -> FSST/ALP kernels | `crates/loom-core/src/l2_kernel_registry.rs`, `crates/loom-core/src/fsst_params.rs`, `crates/loom-core/src/alp_params.rs`, `crates/loom-ffi/src/ffi.rs` | Existing FSST/ALP param tests; planned safety gate runs focused core tests | Planned `scripts/safety-proof-test.sh` | Planned | Add FFI malformed/panic sentinel evidence to matrix after tests. |
