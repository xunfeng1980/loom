...
- Current position: Phase 51 (ABI Freeze, moved from original Phase 44) can freeze ABI fields and ownership rules using Phase 43.2 production-readiness evidence
- Last verified gate: `bash scripts/production-native-codegen-stabilization-test.sh` passed on 2026-06-09 after review fixes; prior `bash scripts/mvp2-verify.sh` passed after Phase 43.2 closeout
...
- [Phase 33]: Complete on 2026-06-09. Default source reports and new `emit_source_ingress_lmc2_*` semantic entry names emit verifier-accepted `LMC2(LMA1)` distribution artifacts.
- [Phase 33]: Historical `emit_source_ingress_lma1_*` entry names emit verifier-accepted direct `LMA1` bridge artifacts for regression evidence after Phase 34 taught DuckDB to scan default `LMC2(LMA1)`.
...
- [Phase 34]: Complete on 2026-06-09. DuckDB `loom_scan(path)` now accepts default `LMC2(LMA1)` artifacts directly through an internal Arrow semantic FFI handle, preserves Arrow field names, supports one-batch multi-column primitive/Utf8/Boolean nullable SQL, and keeps direct `LMA1` as regression-only bridge input.
- [Phase 34]: Date32 logical and Struct nested `LMC2(LMA1)` fixtures are verifier-encoded but rejected by DuckDB SQL with stable `unsupported Arrow semantic schema format` diagnostics; positive logical/nested vector population remains follow-up scope.
...
- Phase 50 added (2026-06-11, PLACEHOLDER): Sidecar Overlay Model and Host-Native Reader Fallback — second slice of the Loom repositioning (整理稿 决定二). Loom becomes a strippable sidecar overlay on unmodified host files (Parquet/Vortex/Lance), content-hash binding host data at column-chunk/fragment granularity to the Phase 49 IR identity; fail-closed routing: integrated+hash-match+supported → verifiable-native track, otherwise → host's own native reader. Existing ingress crates degrade into thin host adapters (one IR + three adapters). No Wasm, no second IR execution / equivalence-diff, no top-level user-facing container (LMC2/LMA1 demote to optional lineage section + dev reference packaging). Stub only — spec via /gsd-spec-phase 50 after Phase 49 lands the IR identity it binds to.
- Phase 49 complete (2026-06-11): Independent L2Core Decode IR Codec and Content-Hash Identity — first slice of the Loom repositioning (整理稿 决定一: decode IR 与 container 分离). Delivered: `l2core_codec.rs` with `L2IR` magic + `u16` version, custom deterministic binary wire format for the full 22-construct L2Core↔kloom.k↔Lean sync surface, zero dependency on `container_codec`/`table_codec`/`layout_codec`/`arrow_semantic_codec` (verified by source grep); `L2CoreProgram::content_hash()` → `l2ir:<hex>` via FNV-1a over canonical bytes; `verify_l2_core_bytes` fail-closed entry point in `full_verifier.rs` that rejects malformed/truncated/bad-discriminant wire form with `ExplicitFailClosed` diagnostic before any facts are produced. All 141 loom-core lib tests pass (including 15 new Phase 49 tests: roundtrip, reencode stability, hash stability, hash collision freedom across diverse corpus, bad magic/version/discriminant/truncated rejection, verify-from-bytes acceptance/rejection parity with in-memory path). Depends on Phase 48. Substrate for Phase 50 sidecar overlay. Non-goals unchanged.
...
- Phase 13 entered: selected full-verifier architecture combines Rust abstract interpretation/type-effect checking, SMT local obligations, and Lean/Rocq semantics scaffolding.
...
- Phase 13 executing: 13-04 complete with Lean `LoomCore.lean` scaffold and `scripts/full-verifier-test.sh`.
...
- Phase 38 remediation complete: `accepted_program_safe` now uses the program-level `verified_program_finishes` / `verified_program_reads_in_bounds` bridge so `Verified p` implies `execProgram p` finishes and every recorded read is in bounds; out-of-bounds reads remain representable as `inBounds := false` for rejected/unverified modeled runs; `scripts/full-verifier-test.sh` rejects `_state : ModeledState`, discarded-premise, direct `And.intro (execProgram p).readSafety`, `readsInBounds`, and row-clamping regressions.
...
- Phase 43.2 inserted: Production Native Codegen Stabilization and Production Readiness is a 44-pre phase after Phase 43.1 and before ABI freeze. It must harden the real native-codegen path for deterministic replay, production-route execution, adversarial output validation, resource/performance behavior, diagnostics/observability, and a Phase 51 ABI-freeze dossier before any ABI is frozen.
- [Phase 43.2 P01]: Added deterministic replay evidence and shape-aware production native-codegen cache keys. Replay evidence now records support fingerprints, output buffer fingerprints, Phase 40 reference/native trace fingerprints, validation fingerprints, runtime cache canonical input, and a combined replay fingerprint. Backend identity and artifact drift alter replay/cache evidence; projection, predicate, row-range split, and mismatched full-scan row counts fail closed or fall back before replay/cache admission, while unsupported or divergent executions cannot produce positive replay evidence.
