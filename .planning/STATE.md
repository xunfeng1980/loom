...
- Phase 49 added (2026-06-11): Independent L2Core Decode IR Codec and Content-Hash Identity — first slice of the Loom repositioning (整理稿 决定一: decode IR 与 container 分离). Make the L2Core decode IR a standalone artifact that can be independently serialized (own codec + magic/version, zero container dependency), independently content-hashed (canonical/deterministic bytes → stable identity), and fail-closed verified from its own wire form. Current gap: `L2CoreProgram` has no Serialize/Deserialize/codec/hash and is never bundled into LMC1/LMC2/LMA1; verifier facts are ephemeral and container-entangled. Depends on Phase 48 (kloom v4 + 22-construct sync checklist) and Phase 36/41 lineage (MD5 digest placeholder). Substrate for Phase 45 artifact-level content-hash. Announced follow-up Phase 50 = 决定二 (sidecar 叠加模型 + 回退宿主原生 reader). Non-goals: no sidecar/host fallback (Phase 50), no signing/remote (45/46), no new constructs, no Wasm/SMT reintroduction, no correctness claims, containers not yet deleted.
