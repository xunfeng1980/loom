...
同一生态中，[F3](https://dl.acm.org/doi/pdf/10.1145/3749163)（ACM SIGMOD）
通过 WebAssembly + AnyBlox runtime 追求相同目标。Loom 走的是 Decode IR
路径——不需要 wasm 沙箱，不需要运行时调度，只是一个可编译为原生代码的总函数 IR。
