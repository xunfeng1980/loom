...
| 验证 | container/layout/table verifier、full-verifier foundation、artifact verifier、SMT-ready constraint IR（证据目标；A–C 阶段无 in-TCB discharge） |
...
| Sidecar overlay | 宿主中立的 `SidecarOverlay`/`ChunkBinding` 数据模型，带 content-hash identity（FNV-1a）；4-gate fail-closed routing（`decide_sidecar_routing`）在 Loom-native track 与 host-native reader fallback 之间做选择；Parquet sidecar 通过 Thrift KeyValue metadata 做 extract/embed；Vortex/Lance adapter 记录格式限制；`loom sidecar embed` CLI |
...
### 13. 运行 sidecar overlay gate

```bash
bash scripts/sidecar-overlay-test.sh
```

这个 gate 验证 Phase 50 sidecar overlay：`SidecarOverlay` 与 `ChunkBinding`
类型具有确定性 encode/decode 和 content-hash identity，Parquet sidecar
extract/embed 通过 Thrift KeyValue metadata 完成 roundtrip，`loom sidecar
embed` CLI 将 sidecar 嵌入 Parquet 文件，strippable overlay invariant 成立
（未知 `loom.*` key 被标准 Arrow reader 忽略），Vortex/Lance adapter 存在并
记录了格式限制。
...
| `crates/loom-container` | 打包/分发层；依赖 `loom-ir-core` |
...
| `ingress/loom-parquet-ingress` | Parquet source ingress，带 sidecar extract/embed（KeyValue metadata） |
| `ingress/loom-vortex-ingress` | Vortex source ingress，带 sidecar adapter |
| `ingress/loom-lance-ingress` | Lance source ingress，带 sidecar adapter |
...
形式化工具和外部工具是显式边界。生产验证保持 oracle-free（不依赖 krun/Bitwuzla）。
Lean/TLC 或 LLVM/MLIR 缺失时，默认不能算通过；只有调用方显式设置相应 opt-out
环境变量时，才允许跳过。
