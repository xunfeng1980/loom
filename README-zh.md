...
| DuckDB | C++ extension 暴露 `loom_scan('<artifact.loom>')`，覆盖 SQL smoke、mixed `LMC1` table payload，以及 Phase 34 primitive/nullable SQL surface 上的默认 `LMC2(LMA1)` Arrow semantic artifacts |
...
| Native execution | production MLIR/LLVM/JIT native codegen 已能为 verifier-accepted `LMC2(LMA1)` / direct `LMA1` 的 one-batch nullable fixed-width primitive artifact 产出 Arrow value/validity buffers 与 RecordBatch；所有被接纳的输出都必须通过 Phase 40 native/model validation，cache identity 绑定 backend/pipeline/trace fingerprints |
