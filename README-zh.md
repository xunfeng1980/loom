...
| Native execution | production MLIR/LLVM/JIT native codegen 已能为 verifier-accepted `LMC2(LMA1)` / direct `LMA1` 的 one-batch nullable fixed-width primitive artifact 产出 Arrow value/validity buffers 与 RecordBatch；这个有界矩阵已有 deterministic replay、adversarial validation、soak/resource evidence，并且所有 cache admission 前都必须通过 Phase 40 native/model validation |
