...
当前仓库已经越过 MVP0,现在处于 **MVP1 / v3 分发与验证阶段**。下文完整设计仍是长期目标;当前范围如下:
...
| MVP1 | 当前已推进到 Phase 20 | `LMT1` 多列、ALP Float32/Float64、fail-closed verifier、`LMC1` container、Safety Proof MVP、Full Verifier foundation、textual MLIR spike、窄范围 native backend evidence、统一 artifact verifier、完整 Vortex reader facts/emission、Bitwuzla-backed SMT discharge evidence,以及第一版 verifier-gated production native-lowering surface seed | Phase 20 不是稳定外部 `L2Core` codec、任意 Vortex encoding/layout 支持、host-engine native execution、checked proof objects、remote/object-store ingress、签名/attestation、完整 correctness proof 或冻结完整 MLIR dialect |
