...
| DuckDB | C++ extension 暴露 `loom_scan('<artifact.loom>')`，用于 SQL smoke coverage；当前 source e2e gate 在 Phase 34 前使用显式 direct-`LMA1` DuckDB bridge fixtures |
...
生成 verifier-accepted `LMC2(LMA1)` distribution artifact，同时写出显式
direct-`LMA1` DuckDB bridge fixtures，并在 DuckDB `loom_scan(...)` 上查询这些
bridge fixtures。
...
full Arrow semantic 和 `LMC2(LMA1)` wrapper gate，然后再运行 DuckDB source
e2e gate。
