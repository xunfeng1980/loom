...
| DuckDB | C++ extension 暴露 `loom_scan('<artifact.loom>')`，用于 SQL smoke coverage，也覆盖 source-backed `LMA1` e2e 切片 |
| Source compatibility | Parquet、Lance、Vortex 只要能由上游 reader materialize 成 Arrow，就可以生成 verifier-accepted `LMA1` semantic artifact |
| Vortex ingress | legacy 窄范围 `.vortex` ingress 仍可将支持的非空 primitive/table case 转成已验证 `LMC1` |
| Native lowering | verifier-gated textual MLIR / decode-dialect evidence，以及 raw primitive lowering 准备 |
...
Loom 将其编码为 `LMA1`，artifact verifier 接受这些 bytes，然后解码出的 `LMA1`
Arrow batches 与 source/oracle Arrow batches 做等价比较。这是 source
compatibility claim，不代表 DuckDB SQL 或 native lowering 已支持所有 Arrow
nested/logical type。

### 7. 运行 DuckDB source e2e gate
...
生成 verifier-accepted 单列 `LMA1` artifact，然后在 DuckDB `loom_scan(...)`
上查询每个 artifact。
...
`scripts/mvp1-verify.sh` 会先运行继承的 `scripts/mvp0-verify.sh`，再运行
