...
| Native execution | engine-neutral native Arrow semantic execution：支持 verifier-accepted `LMC2(LMA1)` / direct `LMA1` 的 one-batch nullable fixed-width primitive artifact，并带 explicit equivalence、runtime/cache identity 与 fail-closed unsupported-shape diagnostics |
