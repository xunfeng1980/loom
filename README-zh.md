...
| MVP1 | 当前已推进到 Phase 21 | `LMT1` 多列、ALP Float32/Float64、失败即关闭的 verifier、`LMC1` container、Safety Proof MVP、Full Verifier foundation、textual MLIR spike、窄范围 native backend evidence、统一 artifact verifier、完整 Vortex reader facts/emission、Bitwuzla-backed SMT discharge evidence、第一版 verifier-gated production native-lowering surface seed,以及带明确 lowering disposition 的真实 Vortex expanded coverage matrix | Phase 21 不是任意 Vortex encoding/layout 支持;任意 Vortex 语义兼容性已预留给 Phase 28。当前也不包含稳定外部 `L2Core` codec、host-engine native execution、checked proof objects、remote/object-store ingress、签名/attestation、完整 correctness proof 或冻结完整 MLIR dialect |
...
- **不规定执行后端**。分发规范只定义“随数据分发的那一层”;如何编译成原生码由各个引擎自行决定(见 §8)。
...
**验证器不证明的**:**正确性**。一个恶意或有 bug 的解码器完全可以安全地、良构地产出**内容错误**的 Arrow。这与今天的原生 reader 一样(原生 reader 也可能解错),因此划到范围外。但必须明确:

> 自解码让你免于“解码器是否会破坏进程”,但**不**让你免于“解码器作者是否可信”。

正确性只能通过正交机制补足(例如输出校验),但通常没有独立的第二份解码结果可对照。因此这里不把正确性作为 Loom verifier 的承诺。
...
2. **正确性不由 verifier 承诺**(§7),必须通过正交机制处理。
