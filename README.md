...
| MVP1 | Reached Phase 20 | `LMT1` multi-column payloads, ALP Float32/Float64, fail-closed verifier, `LMC1` container, Safety Proof MVP, Full Verifier foundation, textual MLIR spike, narrow native-backend evidence, unified artifact verification, complete Vortex reader facts/emission, Bitwuzla-backed SMT discharge evidence, and the first verifier-gated production native-lowering surface seed | Phase 20 is not a stable external `L2Core` codec, arbitrary Vortex encoding/layout support, host-engine native execution, checked proof objects, remote/object-store ingress, signatures/attestation, complete correctness proof, or a frozen complete MLIR dialect |
