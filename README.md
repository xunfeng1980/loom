...
| MVP1 | Reached Phase 18 | `LMT1` multi-column payloads, ALP Float32/Float64, fail-closed verifier, `LMC1` container, Safety Proof MVP, Full Verifier foundation, textual MLIR spike, narrow native-backend evidence, unified artifact verification, and a complete Vortex reader boundary with recursive facts plus supported `.vortex` -> `LMC1`/`LMT1` emission | Phase 18 is not full SMT discharge, a stable external `L2Core` codec, arbitrary Vortex encoding/layout support, a production native compiler, or host-engine native execution; no remote/object-store ingress, signatures/attestation, or complete correctness proof |
